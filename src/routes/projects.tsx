  ...
  head: () => ({
    meta: [
      { title: "Projects — AXIOM Studio" },
      {
        name: "description",
        content:
          "Systems built by AXIOM. AXIOM v1, routing infrastructure, and forward research.",
      },
    ],
  }),
  component: Projects,
...
const PROJECTS = [
  {
    ...
    problem:
      "AI conversations are fragmented, forgetful, and locked to single providers.",
    solution:
      "A unified interface with model routing, persistent memory, and provider-agnostic orchestration.",
    architecture:
      "Edge gateway → intent classifier → routing plane → provider adapters → memory ledger.",
  },
  {
    name: "Ledger",
    status: "Beta",
    ...
    problem:
      "Context windows are ephemeral. Users repeat themselves indefinitely.",
    solution:
      "A per-user memory store with typed episodic and semantic layers, retrievable across agents.",
    architecture:
      "Write-through vector store with typed schemas, TTL zones, and audit trail.",
  },
  {
    name: "Router v2",
    status: "Research",
    ...
    problem:
      "The 'right' model varies per prompt — but users shouldn't choose.",
    solution:
      "A learned router that dispatches by cost, capability, and confidence, per turn.",
    architecture:
      "Feature extractor → policy head → provider fanout with hedging.",
  },
];
    ...
    <PageShell>
      <section className="mx-auto max-w-6xl px-6 pt-8 pb-12">
        ...
        <p className="mt-6 max-w-2xl text-muted-foreground">
          Not a portfolio. A working index of the systems currently running
          under the AXIOM name — and a few we're still teaching to walk.
        </p>
      </section>
        ...
        {PROJECTS.map((p, i) => {
          ...
          return (
            ...
            >
              ...
              {isOpen && (
                <div className="border-t border-border p-6 md:p-8 grid gap-8 md:grid-cols-2 animate-rise">
                  <div>
                  ...
                  <div>
                    ...
                    <div className="mt-6 aspect-video rounded-xl border border-border bg-gradient-to-br from-[var(--graphite)] to-[var(--graphite-2)] grid-bg flex items-center justify-center">
                      <div className="glass-red rounded-full px-4 py-1.5 text-[11px] font-mono">
                        preview.render
                      </div>
                    </div>
                    <div className="mt-6 flex flex-wrap gap-2">
                      <button className="rounded-lg glass-red px-4 py-2 text-xs font-medium">
                        Live Demo
                      </button>
                      <button className="rounded-lg border border-border bg-secondary/80 px-4 py-2 text-xs font-medium">
                        GitHub
                      </button>
                    </div>
                  </div>
