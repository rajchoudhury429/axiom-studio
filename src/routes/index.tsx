...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "The engineering headquarters of AXIOM. A collective of engineers, researchers, and designers building machine intelligence with intention." },
    ],
  }),
  component: Home,
});
    ...
    <div ref={ref} className="relative mx-auto aspect-square w-full max-w-[560px]"
      ...
      {[0.55, 0.72, 0.9].map((s, i) => (
        <div key={i}
          ...
          style={{ transform: `scale(${s})`, animation: `spin-slow ${18 + i * 8}s linear ${i % 2 ? "reverse" : ""} infinite` }}>
          <div className="absolute left-1/2 top-0 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[oklch(0.65_0.22_25)] shadow-[0_0_20px_oklch(0.65_0.22_25)]" />
        </div>
      ))}

      {/* inner glow core */}
      <div className="absolute inset-[26%] rounded-full bg-gradient-to-br from-[oklch(0.7_0.24_25)] via-[oklch(0.35_0.18_25)] to-[oklch(0.12_0.05_20)] blur-[2px]" />
      <div className="absolute inset-[30%] rounded-full glass-red animate-pulse-glow" />
      <div className="absolute inset-[38%] rounded-full bg-[oklch(0.9_0.15_25)] mix-blend-screen opacity-70 blur-md" />
      ...
      {TEAM.map((m, i) => {
        ...
        return (
          <div key={m.name} className="absolute" style={{ left: `${x}%`, top: `${y}%`, transform: "translate(-50%, -50%)" }}>
            <div className="relative">
              <div className="absolute inset-0 -m-1 rounded-full animate-ping-ring bg-[oklch(0.65_0.22_25/40%)]" style={{ animationDelay: `${i * 0.4}s` }} />
            ...
            <svg className="pointer-events-none absolute left-1/2 top-1/2 -z-10 h-[520px] w-[520px] -translate-x-1/2 -translate-y-1/2" viewBox="0 0 100 100">
              <line x1="50" y1="50" x2={50 - Math.cos(a) * r} y2={50 - Math.sin(a) * r}
                stroke="oklch(0.55 0.22 25 / 40%)" strokeWidth="0.2" strokeDasharray="1 1" />
            </svg>
          </div>
        );
      })}
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        {Array.from({ length: 40 }).map((_, i) => (
          <span key={i} className="absolute h-[3px] w-[3px] rounded-full bg-white/30"
            style={{
              left: `${(i * 37) % 100}%`,
              top: `${(i * 19) % 100}%`,
              opacity: 0.15 + ((i * 7) % 60) / 100,
              animation: `float-slow ${5 + (i % 6)}s ease-in-out ${i * 0.15}s infinite`
            }} />
        ))}
      </div>
        ...
        <div className="grid gap-14 lg:grid-cols-[1.05fr_1fr] lg:items-center">
          <div>
            ...
            <p className="mt-8 max-w-xl text-base leading-relaxed text-muted-foreground">
              AXIOM is a collective of five engineers architecting a new kind of AI
              interface — one that reasons, routes and remembers. This is the studio
              behind the system.
            </p>
            <div className="mt-10 flex flex-wrap items-center gap-3">
              <Link to="/projects" className="group relative inline-flex items-center gap-2 rounded-lg glass-red px-5 py-3 text-sm font-medium">
                <span>Enter the Lab</span>
                <span className="transition-transform group-hover:translate-x-1">→</span>
              </Link>
              <Link to="/about" className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-5 py-3 text-sm font-medium hover:bg-white/[0.06] transition">
                Read the Doctrine
              </Link>
            </div>
      ...
      <div className="grid gap-12 lg:grid-cols-[1fr_2fr]">
        <div>
          ...
          <h2 className="hero-title mt-4 text-4xl md:text-5xl">
            Machines should meet us <span className="text-[oklch(0.7_0.22_25)]">where we think.</span>
          </h2>
        </div>
        ...
        <div className="grid gap-10 md:grid-cols-2 md:items-center">
          <div>
            ...
            <p className="mt-4 text-lg leading-relaxed">
              A unified interface that routes prompts across Gemini, Claude, DeepSeek and
              OpenRouter — with memory, orchestration, and a design system built for
              intent, not chatter.
            </p>
            <div className="mt-6 flex flex-wrap gap-2 font-mono text-[11px]">
              {["Next.js", "Node", "MongoDB", "Redis", "Gemini", "Claude"].map(t => (
                <span key={t} className="rounded-md border border-white/10 bg-white/[0.03] px-2.5 py-1">{t}</span>
              ))}
            </div>
          </div>
          <div className="relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[oklch(0.15_0.05_20)] to-[oklch(0.08_0.02_20)]">
            <div className="absolute inset-0 grid-bg opacity-40" />
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="glass-red rounded-full h-32 w-32 animate-pulse-glow flex items-center justify-center font-display text-2xl">AX</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="relative mx-auto max-w-7xl px-6 py-24">
      <motion.div style={{ y }} className="grid gap-4 md:grid-cols-4">
        {[
          ...
          <div key={l} className="glass rounded-2xl p-6 hover:red-glow transition">
...
function CTA() {
  return (
    <section className="relative mx-auto max-w-7xl px-6 py-24">
      <div className="relative overflow-hidden rounded-3xl border border-[oklch(0.55_0.22_25/40%)] bg-gradient-to-br from-[oklch(0.2_0.12_25)] via-[oklch(0.09_0.02_20)] to-black p-12 md:p-20 text-center">
        ...
        <p className="relative mt-6 mx-auto max-w-xl text-muted-foreground">
          We take on a small number of engagements each quarter. Research, systems, and
          the occasional impossible ask.
        </p>
        <Link to="/contact" className="relative mt-10 inline-flex items-center gap-2 rounded-lg glass-red px-6 py-3 text-sm font-medium">
          Open a channel →
        </Link>
      </div>
    </section>
